...
# 3 One that matches a .pgn download link, with flags, for subbing
    'regex_list' : [( 'http://www\.chess\.com/l?i?v?echess/game\?id=[0-9]{8,9}',
                      'http://www\.chess\.com/(?P<gmtype>l?)i?v?echess/game\?id=(?P<id>[0-9]{8,9})',
                      ...
                      ('http://[a-z]{2}\.lichess\.org/[a-zA-Z0-9]{8}(?:#[0-9]*)?(?:/white|/black)?',
                      'http://(?P<lang>[a-z]{2})\.lichess\.org/(?P<id>[a-zA-Z0-9]{8}).*',
                      'http://\g<lang>.lichess.org/\g<id>/pgn')],
